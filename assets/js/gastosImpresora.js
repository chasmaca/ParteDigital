$(document).ready(function($) {
    ...
    if (formulario === 'gastosImpresora') {
        cargarComboPeriodo();
        direccionEnvio = '../dao/select/cargarTableDepartamento.php';
        direccionConsulta = '../dao/select/cargarDatosImpresora.php';
        var cargamos = true;
        ...
        $.when(recuperaPrecioByN(), recuperaPrecioColor()).done(function(precioByN, precioColor) {
            ...
            $.post(direccionEnvio, function(response) {
                    ...
                    if (resultadoGlobal.hasOwnProperty(key)) {
                        ...
                        if (resultadoJSON === '[') {
                            resultadoJSON += '{ \"id\" :' + id +
                                ...
                                ', \"totalByN\": \"<input type=\'text\' name=\'totalByN_' + idCampos + '\' id=\'totalByN_' + idCampos + '\'>\"' +
                                ...
                                ', \"totalColor\": \"<input type=\'text\' name=\'totalColor_' + idCampos + '\' id=\'totalColor_' + idCampos + '\'>\"' +
                                ' }';
                        } else {
                            resultadoJSON += "," +
                                '{ \"id\" :' + id +
                ...
                resultadoJSON += "]";
                $('#listadoGastosImpresoras').bootstrapTable('destroy');
                $('#listadoGastosImpresoras').bootstrapTable({
                    search: true,
                    cache: false,
                    columns: [
                        { field: 'id', title: 'ID' },
                ...
                if (this.id.indexOf('unidadesByN') !== -1) {
                    ...
                    if (validaContenidoImpresora(periodo, dpto, this.value, precioByN, total, tipo)) {
                        valores = "periodo=" + periodo +
                            ...
                            "&tipo=" + tipo;
                        direccionEnvio = '../dao/insert/altaGastosImpresoras.php';
