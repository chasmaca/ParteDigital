...
global $recuperaGastosMaquina, $recuperaGastosMaquinaToday, $mysqlCon;
...
if ($fechasIguales === true) {
    /*Prepare Statement*/
    if ($stmt = $mysqlCon->prepare($recuperaGastosMaquinaToday)) {
    ...
    /*Prepare Statement*/
    if ($stmt = $mysqlCon->prepare($recuperaGastosMaquina)) {
        $stmt->bind_param("ss", $fecha[0], $fecha[1]);
        /*Ejecucion*/
        $stmt->execute();
        /*Almacenamos el resultSet*/
