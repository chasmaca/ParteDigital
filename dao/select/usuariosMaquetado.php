...
function recuperaUsuarios()
{
    ...
    $parametros = "Query:" . $recuperaTodosUsuarios;
    try {
        /*Prepare Statement*/
        if ($stmt = $mysqlCon->prepare($recuperaTodosUsuarios)) {
            ...
            $jsondata["message"] = "Error recuperando usuarios maquetado " . $mysqlCon->error;
            crearLog(__FILE__, __FUNCTION__, $parametros, 'Exception:' . $mysqlCon->error, "error"); 
        }    
    } catch (Throwable $e) {
        $jsondata["success"] = false;
        $jsondata["message"] = "Error recuperando usuarios maquetado " . $e;
...
function recuperaUsuarioAprobador()
{
    ...
    $parametros = "Query:" . $todosAutorizadoresQuery;
    try {
        /*Prepare Statement*/
