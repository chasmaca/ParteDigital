...
/*Prepare Statement*/
if ($stmt = $mysqlCon->prepare($recuperaRole)) {

    /*Ejecucion*/
    $stmt->execute();
    /*Almacenamos el resultSet*/
    $stmt->bind_result($roleId, $roleDesc);

    /*Incluimos las lineas de la consulta en el json a devolver*/
    while ($stmt->fetch()) {
        $tmp = array();
        $tmp["id"] = utf8_encode($roleId);
        $tmp["nombre"] = utf8_encode($roleDesc);
