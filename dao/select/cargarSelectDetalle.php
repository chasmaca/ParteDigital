...
global $recuperaDetalle, $mysqlCon;
...
/*Prepare Statement*/
if ($stmt = $mysqlCon->prepare($recuperaDetalle)) {
    $stmt->bind_param("i", $tipo);

    /*Ejecucion*/
    $stmt->execute();
    /*Almacenamos el resultSet*/
    $stmt->bind_result($detalle_id, $descripcion);

    /*Incluimos las lineas de la consulta en el json a devolver*/
    while ($stmt->fetch()) {
        $tmp = array();
        $tmp["id"] = utf8_encode($detalle_id);
        $tmp["nombre"] = utf8_encode($descripcion);
