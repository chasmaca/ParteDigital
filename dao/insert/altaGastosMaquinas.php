...
if ($periodo != "" && $departamento != "" && $unidades != "" && $precio != "" && $total != ""  && $tipo != "") {
    ...
    if ($tipo == 'Color') {
        insertamosGastosColor($departamento, $unidades, $precio, $total, $fecha, $mes, $anio);
    } else {
        insertamosGastosByN($departamento, $unidades, $precio, $total, $fecha, $mes, $anio);
    }
} else {
 ...
 * @param string $mes          año
 * @param string $anio         año
 * 
 * @return json
 */
function insertamosGastosByN($departamento, $unidades, $precio, $total, $fecha, $mes, $anio)
{
    global $mysqlCon, $sentenciaInsertGastosMaquina;
    ...
    if (existeRegistro($departamento, $anio, $mes)) {
        ...
        if ($stmt = $mysqlCon->prepare($sentenciaInsertGastosMaquina)) {
            ...
            if (!$stmt->execute()) {
                ...
                $jsondata["message"] = "Fallo la ejecucion:" . $sentenciaInsertGastosMaquina;
            } else {
...
{
    global $mysqlCon, $sentenciaUpdateGastosMaquinaByN;
    ...
    if ($stmt = $mysqlCon->prepare($sentenciaUpdateGastosMaquinaByN)) {
        ...
        if (!$stmt->execute()) {
            $jsondata["success"] = false;
            $jsondata["message"] = "Fallo la ejecucion:" . $sentenciaUpdateGastosMaquinaByN;
        } else {
            $jsondata["success"] = true;
 ...
 * @param string $fecha        año
 * @param string $mes          año
 * @param string $anio         mes
 * 
 * @return json
 */
function insertamosGastosColor($departamento, $unidades, $precio, $total, $fecha, $mes, $anio)
{
    global $mysqlCon, $sentenciaInsertGastosMaquina;
...
{
    global $mysqlCon, $sentenciaUpdateGastosMaquinaColor;
    ...
    if ($stmt = $mysqlCon->prepare($sentenciaUpdateGastosMaquinaColor)) {
        ...
        if (!$stmt->execute()) {
            $jsondata["success"] = false;
            $jsondata["message"] = "Fallo la ejecucion:" . $sentenciaUpdateGastosMaquinaColor;
        } else {
            $jsondata["success"] = true;
